...
export default function EnquiryForm({ propertyId }: { propertyId: string }) {
  ...
  if (status === "success") {
    return <div className="p-4 bg-green-100 text-green-800 rounded">Enquiry sent successfully!</div>;
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-8 p-4 border rounded shadow-sm">
      ...
      <div className="flex flex-col gap-3">
        <input name="name" type="text" placeholder="Your Name" className="border p-2 rounded" required />
        <input name="email" type="email" placeholder="Your Email" className="border p-2 rounded" required />
        <textarea name="message" placeholder="I'm interested in..." className="border p-2 rounded" required></textarea>
